...
class Ports(nx.Graph):
    ...
    def add_node(self, node, **attr):
        if not isinstance(node, tuple):
            raise TypeError("Node must be a str representing coordinates of a port.")
        x, y = node
        attr["x"] = x
        attr["y"] = y
    ...
    def query(
        self, terminals: bool = True, cty: str = None, to_cty: str = None, strict=False
    ):
        """
        ...
        if strict:
            ...
            if len(strict_filter) < 1:
                raise KeyError(
                    f"There is no ports for your query terminals:{terminals}, from country:{cty}, to country:{to_cty}, strict:{strict}"
                )
        ...
        if len(filtered_nodes) > 0:
            ...
            filtered_nodes = [
                (n, data) for n, data in filtered_nodes_prev if cty_filter(data, cty)
            ]
        ...
        if len(filtered_nodes) > 0:
            ...
            filtered_nodes = [
                (n, data)
                for n, data in filtered_nodes_prev
                if cty_to_filter(data, to_cty)
            ]
    ...
    def get_preferred_ports(
        self, x, y, ft: FeatureCollection, top=None, include_area_name=False
    ):
        preferred_ports = []
        ...
        if include_area_name:
            sorted_by_second = sorted(
                [
                    (a, b / max(_sum, 1), _update_props(a, c), s_area_name)
                    for a, b, c in preferred_ports
                ],
            ...
            sorted_by_second = sorted(
                [
                    (a, b / max(_sum, 1), _update_props(a, c))
                    for a, b, c in preferred_ports
                ],
                key=lambda tup: tup[1],
                reverse=True,
