...
from ..utils import distance, load_from_geojson
...
class Marnet(nx.Graph):
    ...
    def add_node(self, node, **attr):
        if not isinstance(node, tuple):
            raise TypeError("Node must be a tuple representing the coordinates.")
        x, y = node
        attr["x"] = x
        attr["y"] = y
    ...
    def add_edge(self, u, v, **attr):
        if not isinstance(u, tuple) or not isinstance(v, tuple):
            raise TypeError("Nodes must be tuples representing the coordinates.")
    ...
    def query(self, apply_restrictions=True, restrictions=None):
        ...
        if apply_restrictions:
            ...
            filtered_edges = [
                ...
                if data.get("passage", None) not in restrictions_
            ]
    ...
    def custom_weight(self, u, v, edge_data):
        ...
        avoid_edge = any(
            value.get("passage", None) in (self.restrictions) for value in edge_values
        )
    ...
    def filter_avoid_passages(self, u, v, edge_data, args):
        ...
        avoid_edge = any(
            value.get("passage", None) in (self.restrictions or [])
    ...
    def __custom_w(self, u, v, data):
        return (
            data.get("weight")
            if data.get("passage") not in self.restrictions
            else float("inf")
        )
