...
from .classes import marnet, passages, ports
from .utils import (
    distance_length,
...
def setup_P():
    from .data.ports_dict import edge_list as port_e
    from .data.ports_dict import node_list as port_n
...
@lru_cache(maxsize=None)
def setup_M():
    from .data.marnet_dict import edge_list as marnet_e
    from .data.marnet_dict import node_list as marnet_n
...
def searoute(
    ...
    if include_ports:
        ...
        closestPortOrigin = P.query(
            ...
            cty=country_pol,
            to_cty=to_cty,
            strict=country_restricted_strict,
        ).kdtree.query(origin)
    ...
    """
    ls, traversed_passages = process_route(
        shortest_route_by_distance, M, return_passages
    )
    ...
    feature = Feature(
        ...
        properties={"length": total_length, "units": units, "duration_hours": duration},
    )
    ...
    if return_passages:
        feature.properties["traversed_passages"] = (
            passages.Passage.filter_valid_passages(traversed_passages)
        )
