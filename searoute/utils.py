...
def nearest_node(G, *args, filter=None, fargs=()):
    ...
    for node, data in G.nodes(data=True):
        ...
        for ix, arg in enumerate(args):
            ...
            if filtered or (
                not ignoreEdgeCheck and filter(node, arg, edge_data, fargs)
            ):
                dists[node][ix] = aDist
                found = True
...
def to_nodes_edges_list(G, file_name):
    ...
    with open(file_name, "w") as convert_file:
        convert_file.write(
            f"node_list={str(list(G.nodes(data=True)))}\n\nedge_list={str(list(G.edges(data=True)))}"
        )
...
def validate_lon_lat(coord):
    ...
    if not isinstance(coord, (tuple, list)) or len(coord) != 2:
        raise ValueError(
            "Invalid input format. Must be a tuple or list with two elements as (lan,lon)."
        )
